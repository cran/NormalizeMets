...
After removing missing values and log transforming the data, explore the data using various plots.


### RLA Plots

RLA (Relative Log Abundance) plots are good for initial visualisation. Consistent sized and centered boxes are desirable. See [@DeLivera2012a @DeLivera2015] for details.

```{r samplerla, eval = TRUE, fig.align='center', results='asis', fig.width = 9}
RlaPlots(imp$featuredata, gfactor, saveinteractiveplot = params$saveplots, plotname = "Sample-wise RLA plot")
...
my.pca <- PcaPlots(imp$featuredata, imp$sampledata[,params$factorOI], varplot = FALSE, multiplot = FALSE, interactiveonly = TRUE, interactiveplots = TRUE, userinput = FALSE, returninteractive = TRUE)
my.pca[[1]]
...
Dendrogram(imp$featuredata,imp$sampledata[,params$factorOI])
```
...
HeatMap(imp$featuredata, imp$sampledata[,params$factorOI], return.interactive = TRUE, interactiveplot = TRUE, numeric.mets = TRUE)
...
CompareRlaPlots(resids,groupdata=params$gfactor,
                yrange=c(-3,3),
...
CompareRlaPlots(featuredata.norm, gfactor, normmeth = normmeth.norm, yrange = c(-1,1), plottitle = "Sample-Wise Compare RLA") 
...
ComparePcaPlots(featuredata.norm, list(imp$sampledata[,params$factorOI]), lmain = normmeth.norm)
...
<!-- Exploring whether the normalised data result in expected correlation structures. See [@Freytag2015] for a detailed example of this concept. -->
...
<!-- cor_result[[ii]]<-Corr(normalisedD[[ii]]) -->
<!-- lcor<-cor_result[[ii]]$results[,3] -->
<!-- lcor_p<-cor_result[[ii]]$results[,4] -->
<!-- ComparePvalHist(lcor,normmeth = normmeth.fit, -->
<!--                 xlim=c(-1,1), xlab="Correlation coefficients") -->
<!-- ComparePvalHist(lcor_p,normmeth = normeth.fit, -->
<!--                 xlim=c(0,1)) -->
